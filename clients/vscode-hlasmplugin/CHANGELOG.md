...
## [1.2.0](https://github.com/eclipse/che-che4z-lsp-for-hlasm/compare/tbd)

#### Added

- Allowed viewing content that is generated by the AINSERT instruction and pre-processors
- Expanded the list of associated file extensions
- Added support for the VERSION option in a DB2 preprocessor
- Added support for TODO instruction set versioning
<!-- TODO not sure what that means here -->
- Added support for basic GOFF, XOBJECT, SYSOPT_XOBJECT, and MNOTE
- You can now edit Assembler options in `pgm_conf.json` to override values from `proc_grps.json`
...
- TODO Parsing of negative numbers in machine expressions
<!-- TODO What has been changed/fixed exactly? -->
- Empty arrays now behave similarly to other subscripted variables in macro tracer
- TODO Forward structured macro parameters correctly
<!-- TODO Incomprehensible. At least to an outsider like me -->
- Empty operands are now ignored by the SETx instructions
- Fixed incorrect operator precedence in conditional assembly expressions
- Fixed incomplete conditional assembly expression evaluation
