...
request::request(json message, server* executing_server) :
	message(std::move(message)), valid(true), executing_server(executing_server) {}

request_manager::request_manager(std::atomic<bool>* cancel) :
	end_worker_(false), worker_(&request_manager::handle_request_, this, &end_worker_), cancel_(cancel) {}
...
void request_manager::finish_server_requests(server * to_finish)
{
	...
	requests_.erase(std::remove_if(requests_.begin(), requests_.end(),[&to_finish](auto r){return r.executing_server == to_finish;}), requests_.end());
}
