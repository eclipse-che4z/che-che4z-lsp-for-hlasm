...
{
	ws_mngr_.unregister_debug_event_consumer(*this);
}
...
void feature_launch::register_methods(std::map<std::string, method>& methods)
{
	...
	methods.emplace("scopes", std::bind(&feature_launch::on_scopes, this, std::placeholders::_1, std::placeholders::_2));
	methods.emplace("next", std::bind(&feature_launch::on_next, this, std::placeholders::_1, std::placeholders::_2));
	methods.emplace("stepIn", std::bind(&feature_launch::on_step_in, this, std::placeholders::_1, std::placeholders::_2));
	methods.emplace("variables", std::bind(&feature_launch::on_variables, this, std::placeholders::_1, std::placeholders::_2));
	methods.emplace("continue", std::bind(&feature_launch::on_continue, this, std::placeholders::_1, std::placeholders::_2));

}

json feature_launch::register_capabilities()
{
	return json
	{
...
void feature_launch::on_set_breakpoints(const json & request_seq, const json & args)
{
	...
	if (bpoints_found != args.end())
	{
		for (auto & bp_json : bpoints_found.value())
		{
			...
			breakpoints_verified.push_back(json
				{
					{"verified", true}
				});
		}
...
{
	response_->respond(request_seq, "threads", json
		{
			{"threads", json::array(
				{
					json
...
json source_to_json(parser_library::source source)
{
	return json{ {"path", source.path()} };
}
...
void feature_launch::on_stack_trace(const json & request_seq, const json &)
{
	...
	response_->respond(request_seq, "stackTrace", json
		{
...
void feature_launch::on_scopes(const json & request_seq, const json & args)
{
	...
	response_->respond(request_seq, "scopes", json
		{
			{"scopes", scopes_json}
		});
}
...
void feature_launch::on_variables(const json & request_seq, const json & args)
{
	...
	response_->respond(request_seq, "variables", json
		{
...
void feature_launch::exited(int exit_code)
{
	response_->notify("exited", json
		{
			{"exitCode", exit_code}
		});
}

void feature_launch::terminated()
{
...
void feature_launch::on_continue(const json & request_seq, const json &)
{
	...
	response_->respond(request_seq, "continue", json
		{
