...
server::server(parser_library::workspace_manager & ws_mngr) : language_server::server(ws_mngr)
{
...
void server::respond_error(const json & request_seq, const std::string & requested_command, int, const std::string & err_message, const json & error)
{
	send_message_->reply(json
		{
			{"seq", ++last_seq_},
...
void server::register_methods()
{
	...
	methods_.emplace("initialize", std::bind(&server::on_initialize, this, std::placeholders::_1, std::placeholders::_2));
	methods_.emplace("disconnect", std::bind(&server::on_disconnect, this, std::placeholders::_1, std::placeholders::_2));
}
