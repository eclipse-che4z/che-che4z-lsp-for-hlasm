...
MATCHER_P2(PointerAndSizeEqArray, pointer, size, "") {
	const int actual_size = std::get<1>(arg);
	if (actual_size != size)
		return false;
...
TEST(text_synchronization, did_change_file)
{
	...
	EXPECT_CALL(ws_mngr, did_change_file(StrEq("c:\\test\\one\\blah.txt"), 7, _, 2)).With(Args<2, 3>(PointerAndSizeEqArray(expected1, 2)));
	...
	EXPECT_CALL(ws_mngr, did_change_file(StrEq("c:\\test\\one\\blah.txt"), 7, _, 1)).With(Args<2, 3>(PointerAndSizeEqArray(expected2, 1)));
