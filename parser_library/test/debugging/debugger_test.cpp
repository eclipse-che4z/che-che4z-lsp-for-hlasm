...
bool check_vars(debugger& d, const std::vector<debugging::variable_ptr>& vars, const std::unordered_map<std::string, test_var_value>& exp_vars)
{
...
bool check_step(debugger & d, const std::vector<debugging::stack_frame> & exp_frames, const std::vector<frame_vars>& exp_frame_vars)
{
	auto frames = d.stack_frames();
...
10      MAC 13
	...
	exp_frame_vars.insert(exp_frame_vars.begin(),
		...
		std::unordered_map<std::string, test_var_value>{//macro locals
...
&VARP(30) SETA 1,456,48,7
 LR 1,1
	...
	exp_frame_vars[0].locals.emplace("VARP",
		list{
			{"30", std::make_shared<test_var_value>(1)},
...
B EQU A
	...
	exp_frame_vars[0].ord_syms.emplace("A",
		test_var_value("13", list{
			{"L", std::make_shared<test_var_value>("1")},
			{"T", std::make_shared<test_var_value>("U")}
			}
	));
	exp_frames[0].begin_line = exp_frames[0].end_line = 2;
	EXPECT_TRUE(check_step(d, exp_frames, exp_frame_vars));
...
TEST(debugger, concurrent_next_and_file_change)
{
	...
	std::thread t([&file_manager, &copy1_filename, &chs]() {file_manager.did_change_file(copy1_filename, 0, chs.data(), chs.size()); });
	m.wait_for_stopped();
