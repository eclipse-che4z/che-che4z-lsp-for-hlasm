...
bool assembler_instruction::operands_size_corresponding(const std::vector<const asm_operand*>& to_check, const range & stmt_range, const diagnostic_collector& add_diagnostic) const
{
...
bool assembler_instruction::check_compat_operands(const std::vector<std::unique_ptr<asm_operand>>& input, const std::string& instr_name, const diagnostic_collector& add_diagnostic) const
{
...
bool assembler_instruction::check_flag_operand(const one_operand* input, const std::string& instr_name, const diagnostic_collector& add_diagnostic) const
{
...
bool assembler_instruction::check_process_flag_parameters(const std::vector<std::unique_ptr<asm_operand>>& input, const std::string& instr_name, const diagnostic_collector& add_diagnostic) const
{
	...
	for (size_t i = 0; i < input.size(); i++)
	{
		...
		if (!check_flag_operand(simple_op, instr_name, add_diagnostic) && !is_param_in_vector(simple_op->operand_identifier, other_pair_options))
			return false;
	}
	return true;
}
...
bool assembler_instruction::check_codepage_parameter(const one_operand& input, const diagnostic_collector& add_diagnostic) const
{
	//hexa value
...
bool assembler_instruction::check_fail_parameters(const std::vector<std::unique_ptr<asm_operand>>& input, const std::string& instr_name, const diagnostic_collector& add_diagnostic) const
{
	for (const auto& operand : input)
	{
		...
		else if (is_operand_complex(operand.get()))
		{
			...
			{
				add_diagnostic(diagnostic_op::error_A016_exact(instr_name, current_operand->operand_identifier, 1, operand->operand_range));
				return false;
			}
			if (!is_operand_simple(current_operand->operand_parameters[0].get()))
			{
				add_diagnostic(diagnostic_op::error_A234_FAIL_complex_param_no(instr_name, current_operand->operand_identifier, operand->operand_range));
				return false;
			}
			...
			{
				add_diagnostic(diagnostic_op::error_A235_FAIL_param_number_format(instr_name, current_operand->operand_identifier, current_operand->operand_parameters[0].get()->operand_range));
				return false;
			}
			if (current_operand->operand_identifier == "MNOTE" || current_operand->operand_identifier == "MSG")
			{
				...
				{
					add_diagnostic(diagnostic_op::error_A237_FAIL_severity_message(instr_name, current_operand->operand_identifier, ident->operand_range));
					return false;
				}
			}
			else if (current_operand->operand_identifier == "MAXERRS" || current_operand->operand_identifier == "NOMAXERRS")
			{
				...
				{
					add_diagnostic(diagnostic_op::error_A236_FAIL_MAXXERS_value(instr_name, current_operand->operand_identifier, ident->operand_range));
					return false;
				}
			}
			else
...
bool assembler_instruction::check_pcontrol_parameters(const std::vector<std::unique_ptr<asm_operand>>& input, const std::string& instr_name, const diagnostic_collector& add_diagnostic) const
{
	...
	for (const auto& operand : input)
	{
		...
		if (simple == nullptr || (!is_param_in_vector(simple->operand_identifier, pcontrol_pair_options) && simple->operand_identifier != "ON" && simple->operand_identifier != "OFF"))
		{
...
bool assembler_instruction::check_using_parameters(const std::vector<std::unique_ptr<asm_operand>>& input, const std::string& instr_name, const diagnostic_collector& add_diagnostic) const
{
	for (const auto& operand : input)
	{
		...
		else if (is_operand_complex(operand.get()))
		{
			...
			{ 
				add_diagnostic(diagnostic_op::error_A228_USING_complex_param_no(instr_name, current_operand->operand_identifier, operand->operand_range));
				return false;
			}
			...
			if (current_operand->operand_identifier == "WARN")
			{
				...
				{
					add_diagnostic(diagnostic_op::error_A228_USING_complex_param_no(instr_name,current_operand->operand_identifier, current_operand->operand_parameters[0]->operand_range));
					return false;
				}
				if (!is_byte_value(number))
				{
					add_diagnostic(diagnostic_op::error_A229_USING_WARN_format(instr_name, current_operand->operand_parameters[0]->operand_range));
					return false;
				}
			}
			else if (current_operand->operand_identifier == "LIMIT")
			{
				...
				if (ident.size() >= 3 && ident.front() == 'X' && ident[1] == '\'' && ident.back() == '\'' && is_value_hexa(ident.substr(2, ident.size() - 3)))
				{
					...
					{
						add_diagnostic(diagnostic_op::error_A232_USING_LIMIT_hexa(instr_name, current_operand->operand_range));
						return false;
					}
				}
				else
				{
					...
					catch (...)
					{
						...
						add_diagnostic(diagnostic_op::error_A230_USING_LIMIT_format(instr_name, current_operand->operand_range));
						return false;
					};
					if (number > 4095)
					{
						// the number is specified in decimal but is not lower than the allowed value
						add_diagnostic(diagnostic_op::error_A231_USING_LIMIT_decimal(instr_name, current_operand->operand_range));
						return false;
					}
...
bool assembler_instruction::check_xref_parameters(const std::vector<std::unique_ptr<asm_operand>>& input, const std::string& instr_name, const diagnostic_collector& add_diagnostic) const
{
	auto first = get_simple_operand(input[0].get());
	if (first == nullptr)
	{
...
bool assembler_instruction::check_suprwarn_parameters(const std::vector<std::unique_ptr<asm_operand>>& input, const std::string instr_name, const std::string& op_name, const diagnostic_collector& add_diagnostic) const
{
	std::string option;
	if (op_name == "NOSUP")
		option = "NOSUPRWARN";
...
bool assembler_instruction::check_assembler_process_operand(const asm_operand* input, const diagnostic_collector& add_diagnostic) const
{
	if (is_operand_simple(input)) //operand is simple
	{
		...
		{
			add_diagnostic(diagnostic_op::error_A162_PROCESS_uknown_option(simple_op->operand_identifier, input->operand_range));
			return false;
		}
	}
	else if (is_operand_complex(input)) //operand is complex, check all possible complex operand names
	{
		...
		{
			add_diagnostic(diagnostic_op::error_A162_PROCESS_uknown_option(current_operand->operand_identifier, input->operand_range));
			return false;
		}
