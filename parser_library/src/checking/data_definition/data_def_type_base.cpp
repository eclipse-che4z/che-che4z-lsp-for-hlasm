...
data_def_type::data_def_type(char type, char extension, modifier_spec bit_length_spec, modifier_spec length_spec,
	modifier_spec scale_spec, modifier_spec exponent_spec, nominal_value_type nominal_type, context::alignment implicit_alignment,
	implicit_length_t implicit_length)
	: type(type), extension(extension), nominal_type(nominal_type), bit_length_spec_(bit_length_spec),
	length_spec_(length_spec), ds_length_spec_(length_spec), ds_bit_length_spec_(bit_length_spec), scale_spec_(scale_spec),
	exponent_spec_(exponent_spec), alignment_(implicit_alignment), implicit_length_(implicit_length)
{
...
data_def_type::data_def_type(char type, char extension, modifier_spec bit_length_spec, modifier_spec length_spec,
	int max_ds_length_spec, modifier_spec scale_spec, modifier_spec exponent_spec, nominal_value_type nominal_type,
	context::alignment implicit_alignment, implicit_length_t implicit_length)
	: type(type), extension(extension), nominal_type(nominal_type),
	bit_length_spec_(bit_length_spec), length_spec_(length_spec), ds_length_spec_(modifier_bound{ 1, max_ds_length_spec }),
	ds_bit_length_spec_(modifier_bound{ 1, max_ds_length_spec * 8 }), scale_spec_(scale_spec),
	exponent_spec_(exponent_spec), alignment_(implicit_alignment), implicit_length_(implicit_length)
{
	type_str = init_type_str(type, extension);
}
...
//and may have byte length 1 to 65535 and bit lenght 1 to 65535*8 with DS. It does not take scale or exponent modifiers.
//It takes nominal value enclosed in apostrophes, has alignment to 1 byte and implicit length is as needed.
// data_def_type('X', '\0', modifier_bound{ 1, 2048 }, modifier_bound{ 1, 256 }, 65535, n_a(), n_a(), nominal_value_type::STRING, no_align, as_needed()) {}
...
bool data_def_type::expects_single_symbol() const
{
	return false;
}
...
bool check_modifier(const data_def_field<data_definition_operand::num_t>& modifier, const std::string& type_str,
	const std::string& modifier_name, modifier_spec bound, const diagnostic_collector& add_diagnostic)
{
	...
	if (modifier.value < std::get<modifier_bound>(bound).min || modifier.value > std::get<modifier_bound>(bound).max)
	{
		...
		add_diagnostic(diagnostic_op::error_D008(modifier.rng, type_str, modifier_name,
			std::get<modifier_bound>(bound).min, std::get<modifier_bound>(bound).max));
		return false;
	}
	return true;
}
...
bool data_def_type::check_dupl_factor(const data_definition_operand& op, const diagnostic_collector& add_diagnostic) const
{
...
{
	if (std::holds_alternative<n_a>(bit_length_spec_) && op.length.len_type == data_def_length::length_type::BIT)
	{
	...
	{
		if (op.length.len_type == data_def_length::length_type::BIT)
...
bool data_def_type::check_nominal_type(const data_definition_operand& op, const diagnostic_collector& add_diagnostic) const
{
	bool ret = true;
	switch (nominal_type)
	{
	...
	case nominal_value_type::EXPRESSIONS:
		...
		for (auto& p : std::get<nominal_value_expressions>(op.nominal_value.value))
			if (std::holds_alternative<data_def_address>(p))
			{
				...
				add_diagnostic(diagnostic_op::error_D020({ adr.displacement.rng.start, adr.base.rng.end }, type_str));
				ret = false;
			}
		if (!ret)
			return false;
...
template<data_instr_type instr_type>
std::pair<bool, bool> data_def_type::check_base(const data_definition_operand& op, const diagnostic_collector& add_diagnostic) const
{
...
bool data_def_type::check(const data_definition_operand&, const diagnostic_collector&, bool) const
{
	return true;
}
...
uint64_t data_def_type::get_length(const data_definition_operand& op) const
{
	...
	if (op.length.present)
	{
		...
		if (op.length.len_type == data_def_length::BYTE)
			len_in_bits *= 8;
	}
...
template bool data_def_type::check<data_instr_type::DC>(const data_definition_operand& op, const diagnostic_collector& add_diagnostic) const;
template bool data_def_type::check<data_instr_type::DS>(const data_definition_operand& op, const diagnostic_collector& add_diagnostic) const;
