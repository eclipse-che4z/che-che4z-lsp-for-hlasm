...
address_operand::address_operand(address_state state, int displacement, int first, int second) :
	state(state), displacement(displacement), first_op(first), second_op(second), op_state(operand_state::PRESENT) {};
...
bool address_operand::check(diagnostic_op& diag, const machine_operand_format to_check, const std::string& instr_name, const range & stmt_range) const
{
	...
	else
	{
		...
		else
		{
			...
			if (op_state != operand_state::FIRST_OMITTED)
			{
				...
				if (to_check.first.type == machine_operand_type::LENGTH && !is_length_corresponding(first_op, to_check.first.size))
				{
					diag = get_first_parameter_error(to_check.first.type, instr_name, 0, 1ll << to_check.first.size, stmt_range);
					return false;
				}
...
bool hlasm_plugin::parser_library::checking::address_operand::is_length_corresponding(int param_value, int length_size) const
{
...
hlasm_plugin::parser_library::diagnostic_op machine_operand::get_simple_operand_expected(const machine_operand_format & op_format, const std::string & instr_name, const range& stmt_range) const
{
