...
struct content_pos
{
	content_pos(size_t line, std::vector<std::string>* text) : line(line), text(text),defined(false) {};
	content_pos() : line(0),text(nullptr), defined(true) {};
...
public:
	...
	completion_item_s(std::string label, std::string detail, std::string insert_text, std::vector<std::string> contents);
...
{	
	...
	// constructor with addition of the completion item describing the instruction and the version of the instruction (macros only)
