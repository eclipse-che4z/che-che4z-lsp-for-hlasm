...
size_t definition::hash() const
{
	return (size_t)name;
}

std::vector<std::string> definition::get_value() const
{
	return { "Defined in file: " + *file_name };
}
...
void hlasm_plugin::parser_library::context::instr_definition::init(const std::string* file, const std::string* n, const range& r)
{
	file_name = file;
	name = n;
	definition_range = r;
...
std::vector<std::string> ord_definition::get_value() const
{
	...
	if (attr->is_defined(context::data_attr_kind::T))
		result.push_back("T: " + ebcdic_encoding::to_ascii((unsigned char)attr->get_attribute_value(context::data_attr_kind::T)));

	return result;
}

size_t ord_definition::hash() const
{
	return (size_t)name;
}

bool ord_definition::operator==(const ord_definition& other) const
{
	return name == other.name;
}
...
size_t instr_definition::hash() const
{
	return (size_t)name * PRIME1 + version;
}
...
bool macro_id::operator==(const macro_id& other) const
{
	return name == other.name && version == other.version;
}
