...
bool location_counter::has_unresolved_spaces() const
{
	return !!org_data_.back().fist_space();
}

size_t location_counter::storage() const
{
	return curr_data().storage;
}

location_counter::location_counter(id_index name, const section& owner, const loctr_kind kind, id_storage& ids)
	: switched_(nullptr), last_space_(0), ids_(ids), layuot_created_(false), name(name), owner(owner), kind(kind)
{
...
aligned_addr location_counter::align(alignment align)
{
...
bool location_counter::need_space_alignment(alignment align) const
{
	return curr_data().need_space_alignment(align);
}
...
space_ptr location_counter::set_value(const address& addr, size_t boundary, int offset, bool has_undefined_part)
{
	...
	if (!addr.in_same_loctr(curr_addr) ||
		(!addr.has_dependant_space() && addr.offset + al + offset < 0))
	...
	if (curr_addr.spaces != addr.spaces ||
		(int)curr_data().storage - addr.offset > curr_data().current_safe_area ||
...
std::pair<space_ptr, std::vector<address>> location_counter::set_available_value()
{
	...
	if (org_data_.size() == 1 && (org_data_.begin()->unknown_parts.empty() || (org_data_.begin()->unknown_parts.size() == 1 && kind == loctr_kind::NONSTARTING)))
...
location_counter_data& location_counter::curr_data()
{
	return org_data_.back();
}

const location_counter_data& location_counter::curr_data() const
{
	return org_data_.back();
}
...
bool location_counter::has_alignment(alignment align) const
{
	return curr_data().has_alignment(align);
}
