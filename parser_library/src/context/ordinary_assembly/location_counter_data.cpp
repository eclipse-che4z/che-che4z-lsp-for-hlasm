...
location_counter_data::location_counter_data(loctr_data_kind kind)
	: storage(0), initial_storage(0), current_safe_area(0), kind(kind) {}
...
bool location_counter_data::has_alignment(alignment align) const
{
	...
	else
	{
		if (align.boundary > unknown_parts.back().unknown_space->align.boundary)
			return false;
		test = unknown_parts.back().unknown_space->align.boundary + unknown_parts.back().unknown_space->align.byte + unknown_parts.back().storage_after;
	}
