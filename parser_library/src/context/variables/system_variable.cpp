...
system_variable::system_variable(id_index name, macro_data_ptr value, bool is_global)
	: macro_param_base(macro_param_type::SYSTEM_TYPE, name, is_global), data_(std::move(value)) {}

const C_t& system_variable::get_value(const std::vector<size_t>& offset) const
{
	return get_data(offset)->get_value();
}

const C_t& system_variable::get_value(size_t idx) const
{
	return macro_param_base::get_value(idx + 1);
}

const C_t& system_variable::get_value() const
{
	return macro_param_base::get_value(0);
}
...
const macro_param_data_component* system_variable::real_data() const
{
	return &*data_;
}
