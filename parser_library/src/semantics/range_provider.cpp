...
range range_provider::union_range(const range& lhs, const range& rhs)
{
	...
	if (lhs.start.line == rhs.start.line)
		...
		ret[0] = (lhs.start.line == std::min(lhs.start.line, rhs.start.line)) ?
			lhs.start : rhs.start;

	if (lhs.end.line == rhs.end.line)
		ret[1] = position(lhs.end.line, std::max(lhs.end.column, rhs.end.column));
	else
		ret[1] = (lhs.end.line == std::max(lhs.end.line, rhs.end.line)) ?
		lhs.end : rhs.end;
...
range_provider::range_provider(range original_range, adjusting_state state)
	: original_range(original_range), state(state) {}

range_provider::range_provider(range original_field_range, std::vector<range> original_operand_ranges, adjusting_state state)
	: original_range(original_field_range), original_operand_ranges(std::move(original_operand_ranges)), state(state) {}
