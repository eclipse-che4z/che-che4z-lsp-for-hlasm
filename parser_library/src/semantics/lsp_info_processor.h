...
class lsp_info_processor
{
    ...
    private:
        ...
        // within a given set of symbols, checks whether it contains a symbol on a given position and returns its definition's position 
        template<typename T>
        bool find_definition_(const position& pos, const context::definitions<T>& symbols, position_uri_s& found) const;
        // within a given set of symbols, checks whether it contains a symbol on a given position and returns all of its occurences
        template<typename T>
