...
class debug_lib_provider : public workspace::parse_lib_provider
{
	const workspace::workspace& ws_;
public:
	debug_lib_provider(const workspace::workspace& ws) : ws_(ws) {}
	...
	virtual workspace::parse_result parse_library(const std::string& library, context::hlasm_context& hlasm_ctx, const workspace::library_data data) override
	{
		...
		{
			std::shared_ptr<workspace::processor> found = lib->find_file(library);
			if (found)
