...
context::SET_t character_expression::get_set_value() const
{
	return value_;
}

character_expression::character_expression(const character_expression & expr) : value_(expr.value_)
{
	if (expr.diag)
...
void character_expression::append(std::string v)
{
	value_.append(v);
}
