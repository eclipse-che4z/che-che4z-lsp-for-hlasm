...
void expression::copy_diag(expr_ref o)
{
	diag = std::make_unique< diagnostic_op>(*o->diag);
}

void expression::copy_diag(const expression * o)
{
	diag = std::make_unique< diagnostic_op>(*o->diag);
}
...
expr_ptr expression::resolve_ord_symbol(str_ref symbol)
{
	...
	if (std::all_of(symbol.cbegin(), symbol.cend(),
...
expr_ptr expression::evaluate_term(std::deque<expr_ptr>& exprs, uint8_t priority, size_t& operator_count)
{
	...
	if (exprs.size() > 0
		&& exprs.front()->is_keyword()
		&& front_keyword(exprs)->priority() == priority)
	{
...
expr_ptr expression::evaluate_factor(std::deque<expr_ptr>& exprs, size_t& operator_count)
{
	...
	//value
	if (!exprs.front()->is_keyword()
		|| (exprs.front()->is_keyword() && exprs.size() == 1))
	{
	...
	if (exprs.front()->is_keyword() &&
		dynamic_cast<keyword_expression *>(exprs.front().get())->is_unary())
	{
		++operator_count;
		if (operator_count > 24)
			return default_expr_with_error<arithmetic_expression>
...
context::SET_t expression::get_set_value() const
{
