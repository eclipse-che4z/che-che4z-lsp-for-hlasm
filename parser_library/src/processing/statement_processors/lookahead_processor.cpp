	...
	if (instruction.type == semantics::instruction_si_type::ORD)
	{
		...
		if (hlasm_ctx.macros().find(opcode) != hlasm_ctx.macros().end())
		{
			return std::make_pair(
				processing_format(processing_kind::LOOKAHEAD, processing_form::IGNORED),
...
lookahead_processor::lookahead_processor(
	...
	macro_nest_(0), branch_provider_(branch_provider),
	listener_(listener), lib_provider_(lib_provider),
	to_find_(std::move(start.targets)), target_(start.target), action(start.action),
...
lookahead_processor::process_table_t lookahead_processor::create_table(context::hlasm_context& ctx)
{
	...
	table.emplace(ctx.ids().add("DC"),
		std::bind(&lookahead_processor::assign_data_def_attributes, this, std::placeholders::_1, std::placeholders::_2));
...
void lookahead_processor::assign_EQU_attributes(context::id_index symbol_name, const resolved_statement& statement)
{
	...
	if (statement.operands_ref().value.size() >= 3 && statement.operands_ref().value[2]->type == semantics::operand_type::ASM)
	{
	...
	if (statement.operands_ref().value.size() >= 2 && statement.operands_ref().value[1]->type == semantics::operand_type::ASM)
	{
	...
	{
		if (statement.operands_ref().value.size() >= 1 && statement.operands_ref().value[0]->type == semantics::operand_type::ASM)
		{
...
void lookahead_processor::assign_assembler_attributes(context::id_index symbol_name, const resolved_statement& statement)
{
