...
public:
	...
	lookahead_processor(context::hlasm_context& hlasm_ctx, branching_provider& branch_provider, processing_state_listener& listener, const lookahead_start_data start);
...
private:
	...
	void find_target(const semantics::complete_statement& statement);
	void find_seq(const semantics::complete_statement& statement);
};
