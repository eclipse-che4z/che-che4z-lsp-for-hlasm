...
context::SET_t context_manager::get_var_sym_value(context::id_index name,
    ...
    const std::string& symbol, const range& symbol_range, diagnostic_op_consumer& diags) const
{
    ...
    if (!valid)
        diags.add_diagnostic(diagnostic_op::error_E065(symbol_range));
    return id;
}

bool context_manager::test_symbol_for_read(const context::var_sym_ptr& var,
    const std::vector<context::A_t>& subscript,
    const range& symbol_range,
    diagnostic_op_consumer& diags) const
{
...
bool context_manager::test_syslist_for_read(
