...
context::id_index context_manager::concatenate(
    const semantics::concat_chain& chain, expressions::evaluation_context eval_ctx) const
{
    return hlasm_ctx.ids().add(concatenate_str(chain, eval_ctx));
}

std::string context_manager::concatenate_str(
    const semantics::concat_chain& chain, expressions::evaluation_context eval_ctx) const
{
    expressions::expression_evaluator evaluator(eval_ctx);
...
context::macro_data_ptr context_manager::create_macro_data(
    const semantics::concat_chain& chain, expressions::evaluation_context eval_ctx) const
{
    expressions::expression_evaluator evaluator(eval_ctx);
...
context::SET_t context_manager::get_var_sym_value(
    const semantics::var_sym& symbol, expressions::evaluation_context eval_ctx) const
{
    auto id =
    ...
    context::var_sym_ptr var, const expressions::expr_list& subscript, const range& symbol_range) const
{
    ...
    else if (auto mac_par = var->access_macro_param_base())
    {
        for (size_t i = 0; i < subscript.size(); ++i)
        {
            ...
            {
                if (i == 0 && e && e->get_numeric_value() == 0
                    && dynamic_cast<context::system_variable*>(mac_par)) // if syslist and subscript = 0, ok
                    continue;
